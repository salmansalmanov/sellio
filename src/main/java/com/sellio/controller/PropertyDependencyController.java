...
public class PropertyDependencyController {
    ...
    @PostMapping
    @Operation(
            summary = "Create property dependency",
            responses = {
                    @ApiResponse(description = "Success", responseCode = "201")
            }
    )
    public ResponseEntity<DataResult<PropertyDependencyDetailsResponse>> createPropertyDependency(@RequestBody @Valid PropertyDependencyRequest propertyDependencyRequest) {
        return new ResponseEntity<>(propertyDependencyService.save(propertyDependencyRequest), HttpStatus.CREATED);
    }

    @GetMapping("/{id}")
    @Operation(
            summary = "Get property dependency by ID",
            responses = {
    ...
    @GetMapping
    @Operation(
            summary = "Get all property dependencies",
            responses = {
                    @ApiResponse(description = "Success", responseCode = "200")
            }
    )
    ...
    @DeleteMapping("/{id}")
    @Operation(
            summary = "Delete property dependency by ID",
            responses = {
                    @ApiResponse(description = "Success", responseCode = "200")
            }
    )
